	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		if (exchange.getRequestMethod().toUpperCase().equals("POST")) {
			if (HandlerUtils.authorizeUser(exchange)) {
				...
				if (success) {
					Server.println("  Successful request to /moves/Year_of_Plenty.");
					HandlerUtils.sendStringAsJSON(exchange, HttpURLConnection.HTTP_OK, Games.get().getGames().get(gameID).toString());
				} else {
					Server.println("  Invalid request to /moves/Year_of_Plenty.");
				...
				Server.println("  Unauthorized request to /moves/Year_of_Plenty.");
